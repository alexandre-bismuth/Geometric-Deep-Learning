...
def plot_vnode_comparison(agg_vnode, agg_no_vnode, layers, metric_name, title, save_path=None):
    fig, ax = plt.subplots(figsize=(8, 5))

    for agg, label, color in [(agg_vnode, 'With VNode', 'tab:red'), (agg_no_vnode, 'Without VNode', 'tab:blue')]:
        if metric_name in agg:
            mean = agg[metric_name]['mean']
            std = agg[metric_name]['std']
            valid = ~np.isnan(mean)
